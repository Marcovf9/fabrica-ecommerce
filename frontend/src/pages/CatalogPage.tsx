...
function CatalogPage() {
  ...
  // Estado estructurado del cliente
  ...
  const addToCart = (product: Product) => {
    if (product.availableStock <= 0) {
      return alert(`El producto ${product.name} está agotado.`);
    }
    ...
    setCart((prevCart) => {
      ...
      if (existing) {
        if (existing.quantity >= product.availableStock) {
          alert(`Solo quedan ${product.availableStock} unidades de ${product.name} en stock.`);
          return prevCart;
        }
  ...
  // Extraemos las categorías únicas para agrupar visualmente
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif', display: 'flex', gap: '40px', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      <div style={{ flex: 1, backgroundColor: '#f8f9fa', padding: '25px', borderRadius: '12px', height: 'fit-content', border: '1px solid #e9ecef' }}>
              ...
              <li key={item.product.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px', paddingBottom: '15px', borderBottom: '1px solid #eee' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <button 
                    ...
                    style={{ backgroundColor: '#f39c12', color: 'white', border: 'none', borderRadius: '4px', width: '25px', height: '25px', cursor: 'pointer', fontWeight: 'bold' }}>
                    -
                  </button>
                  <span style={{ fontWeight: 'bold', minWidth: '20px', textAlign: 'center' }}>{item.quantity}</span>
                  <button 
                    onClick={() => addToCart(item.product)}
                    style={{ backgroundColor: '#2ecc71', color: 'white', border: 'none', borderRadius: '4px', width: '25px', height: '25px', cursor: 'pointer', fontWeight: 'bold' }}>
                    +
                  </button>
                  <button 
                    onClick={() => removeFromCart(item.product.id)}
                    style={{ backgroundColor: '#e74c3c', color: 'white', border: 'none', borderRadius: '4px', width: '25px', height: '25px', cursor: 'pointer', fontWeight: 'bold', marginLeft: '10px' }}
        ...
        <h3 style={{ fontSize: '1.5rem', marginTop: '20px', borderTop: '2px solid #2c3e50', paddingTop: '15px' }}>
        ...
        <div style={{ marginTop: '30px' }}>
          ...
          <button 
            onClick={submitOrder}
            style={{ width: '100%', padding: '15px', backgroundColor: '#2ecc71', color: 'white', border: 'none', borderRadius: '6px', fontSize: '1.1rem', fontWeight: 'bold', cursor: 'pointer' }}>
            Generar Pedido
          </button>
        </div>
      </div>
